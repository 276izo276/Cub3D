...
static void	check_dir(t_data *data, int i, int y, int x)
{
	...
	data->ray[i].texture_coo.x = (int)(posx_display * data->map.msg_img[data->map.wall_map[y][x]->msg_nb]->width);
	if (data->ray[i].texture_coo.x < 0)
		data->ray[i].texture_coo.x = 0;
	if (data->ray[i].texture_coo.x >= data->map.msg_img[data->map.wall_map[y][x]->msg_nb]->width)
		data->ray[i].texture_coo.x = data->map.msg_img[data->map.wall_map[y][x]->msg_nb]->width - 1;
}

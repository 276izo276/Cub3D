...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   spell_whithout_text.c                              :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: ale-guel <ale-guel@student.42lyon.fr>      +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/12/10 07:46:56 by ale-guel          #+#    #+#             */
/*   Updated: 2025/12/10 07:46:57 by ale-guel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	get_dark_color(t_data *data, int i, double dist, double factor)
{
	int				r;
	int				g;
	int				b;
	...
	b = (color & 255);
	b = (int)(b - b * ((1 - dist * factor)));
	if (b > 255)
		b = 255;
	g = (color >> 8 & 255);
	g = (int)(g - g * ((1 - dist * factor)));
	if (g > 255)
		g = 255;
	r = (color >> 16 & 255);
	r = (int)(r - r * ((1 - dist * factor)));
	if (r > 255)
		r = 255;
	color = (r << 16) + (g << 8) + b;
...
static void	calc_bh_dist(t_data *data, int i, int j, double *dist)
{
	double	rap_y;

	rap_y = (data->ray[i].pix_y - data->mlx.height / 2)
		/ ((data->ray[i].items[j]->hbot - data->ray[i].items[j]->htop) * (80.0
				/ 90.0));
...
void	display_bh(t_data *data, int i, int j)
{
	...
	while (data->ray[i].pix_y < data->ray[i].items[j]->hbot
		...
		&& data->ray[i].pix_y < round(data->ray[i].max_height
			+ data->ray[i].items[j]->max_size))
	{
		calc_bh_dist(data, i, j, &dist);
		if (dist < .5)
			get_dark_color(data, i, dist, 0.5);
		data->ray[i].pix_y++;
	}
}

static void	calc_anim_death_dist(t_data *data, int i, int j, double *dist)
{
	double	rap_y;

	rap_y = (data->ray[i].pix_y - data->mlx.height / 2)
		/ ((data->ray[i].items[j]->hbot - data->ray[i].items[j]->htop));
	*dist = sqrt((data->ray[i].items[j]->posx - 0.5)
			* (data->ray[i].items[j]->posx - 0.5) + rap_y * rap_y);
}
...
void	display_anim_death(t_data *data, int i, int j)
{
	...
	while (data->ray[i].pix_y < data->ray[i].items[j]->hbot
		...
		&& data->ray[i].pix_y < round(data->ray[i].max_height
			+ data->ray[i].items[j]->max_size))
	{
		calc_anim_death_dist(data, i, j, &dist);
		if (dist < .5)
			get_dark_color(data, i, dist, 0.1);
		data->ray[i].pix_y++;
	}
}

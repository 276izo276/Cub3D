...
static unsigned int	get_color(t_data *data, int text_x, int text_y, char c)
{
	...
	color = 0;
	if (c == '1')
		color = get_texture_pixel(&data->map.mini.img[MAP_WALL],
				text_x, text_y);
	else if (c == 'D')
		color = get_texture_pixel(&data->map.mini.img[MAP_DOOR],
				text_x, text_y);
	else if (c == 'F')
	{
		color = get_texture_pixel(&data->map.mini.img[MAP_FLOO],
				text_x, text_y);
		if (data->status != MAP && data->map.is_center == false)
			color = darken_the_color(color);
	}
	else if (c == '0' || c == 'W' || c == 'S' || c == 'N' || c == 'E')
		color = get_texture_pixel(&data->map.mini.img[MAP_FLOOR],
				text_x, text_y);
	else if (c != '.')
		color = 0x6e583e;
	return (color);

...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/12/10 07:48:01 by ale-guel          #+#    #+#             */
/*   Updated: 2025/12/10 07:48:02 by ale-guel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	init_sorcerer_p1(t_data *data, t_enemy *enemy, char c)
{
	if (c == KUNFANDI)
	{
		...
		enemy->life = 150 + (data->sorcerer[2].nb_spawn * 25);
	...
	if (c == CAMEO)
	{
		...
		enemy->life = 150 + (data->sorcerer[4].nb_spawn * 25);
		enemy->color_coa = AIR_COLOR;
		enemy->damage.which_coa_do = AIR;
	}
}
...
static void	init_sorcerer_p2(t_data *data, t_enemy *enemy, char c)
{
	if (c == NANCY)
	{
		...
		enemy->life = 250 + (data->sorcerer[0].nb_spawn * 25);
	...
	if (c == ANAIS)
	{
		...
		enemy->life = 150 + (data->sorcerer[5].nb_spawn * 25);
		enemy->color_coa = AIR_COLOR;
		enemy->damage.which_coa_do = AIR;
	}
}
...
static void	init_sorcerer_p3(t_data *data, t_enemy *enemy, char c)
{
	if (c == PILO)
	{
		...
		enemy->life = 150 + (data->sorcerer[3].nb_spawn * 25);
	...
	if (c == DIRLO)
	{
		...
		enemy->life = 150 + (data->sorcerer[6].nb_spawn * 25);
		enemy->color_coa = EARTH_COLOR;
		enemy->damage.which_coa_do = EARTH;
	}
}
...
static void	init_sorcerer_p4(t_data *data, t_enemy *enemy, char c)
{
	if (c == ZIPPY)
	{
		...
		enemy->life = 200 + (data->sorcerer[1].nb_spawn * 25);
	...
	if (c == STEF)
	{
		...
		enemy->life = 200 + (data->sorcerer[7].nb_spawn * 25);
		enemy->color_coa = FIRE_COLOR;
		enemy->damage.which_coa_do = FIRE;
	}
}
...
void	fill_right_img_sorcerer(t_data *data, t_enemy *enemy, char c)
{
	...
	if (c == MARINA)
	{
		...
		enemy->life = 150 + (data->sorcerer[8].nb_spawn * 25);
		enemy->color_coa = EARTH_COLOR;
		enemy->damage.which_coa_do = EARTH;
	}

...
{
	if (type == MARINA || type == CAMEO || type == DIRLO || type == NANCY || type == PILO || type == ZIPPY || type == ANAIS || type == STEF || type == KUNFANDI)
		return (true);
	return (false);
}
		...
		if (enemy->dist_target < enemy->dist_visu)
		{
				...
				get_mtime() > enemy->time_attack_dist + enemy->cooldown_dist * 1000)
			{
				...
				data->item = add_end_lst(create_item(data, FIREBALL_ELEM, 
					&enemy->center, enemy->aff_deg + 180), data->item, f_item);
			}
...
// 		}
// 	}
// 	else if (enemy->type == SNAKE || is_sorcerer(data, enemy->type) == true)
// 	{
// 		t_lst	*lst;
// 		t_enemy	*elem;
// 		t_enemy *keep_elem;
...
// 		{
// 			elem = lst->dt;
// 			if (elem->type == SNAKE || (is_sorcerer(data, elem->type) == true && elem->color_coa == enemy->color_coa))
// 			{
// 				lst = lst->next;
// 				continue;
// 			}
...
// 			if (enemy->dist_target < enemy->dist_visu)
// 			{
// 				if (is_sorcerer(data, enemy->type) == true && rand() % 1000 < 50 && get_mtime() > enemy->time_attack_dist + enemy->cooldown_dist * 1000)
// 				{
// 					enemy->time_attack_dist = get_mtime();

...
void	put_top_color(t_data *data, int x, int y, unsigned int *color)
{
	if (y > 175)
		return ;
	if (*color == YELLOW && x < data->img[SANDGLASS].width / 3)
		*color = data->coa[FIRE].color;
	else if (*color == YELLOW && x < data->img[SANDGLASS].width / 2)
		*color = data->coa[WATER].color;
	else if (*color == YELLOW && x < data->img[SANDGLASS].width - data->img[SANDGLASS].width / 3)

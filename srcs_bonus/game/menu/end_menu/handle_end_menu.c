...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/12/10 07:49:32 by ale-guel          #+#    #+#             */
/*   Updated: 2025/12/10 07:49:33 by ale-guel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	put_top_color(t_data *data, int x, int y, unsigned int *color)
{
	...
	if (*color == YELLOW)
	{
		...
		else if (x < data->img[SANDGLASS].width - data->img[SANDGLASS].width
			/ 3)
...
void	fill_score_color(t_data *data, int x, int y, unsigned int *color)
{
	...
	if (*color == YELLOW)
	{
		if (x < data->img[SANDGLASS].width / 3 && y >= data->coa[FIRE].max_y)
			*color = data->coa[FIRE].color;
		if (x > data->img[SANDGLASS].width / 3 && x < data->img[SANDGLASS].width
			/ 2 && y >= data->coa[WATER].max_y)
			*color = data->coa[WATER].color;
		if (x > data->img[SANDGLASS].width / 2 && x < data->img[SANDGLASS].width
			- data->img[SANDGLASS].width / 3 && y >= data->coa[EARTH].max_y)
			*color = data->coa[EARTH].color;
		if (x > data->img[SANDGLASS].width - data->img[SANDGLASS].width / 3
			&& x < data->img[SANDGLASS].width && y >= data->coa[AIR].max_y)
			*color = data->coa[AIR].color;
	}
}

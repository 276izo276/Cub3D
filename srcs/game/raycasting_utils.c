...
int	handle_ray_y_top(t_data *data, int i)
{
	...
	{
		data->ray[i].final_y = 0;
		data->ray[i].final_x = data->ray[i].ry * data->ray[i].delta_x;
		data->ray[i].final_x = round(data->ray[i].coo_x * 64) / 64.0;
		// for (int i = -2; i < 2; i++)
		// {
		// 	for (int j = -2; j < 2; j++)
		// 	{
		// 		x = i + (5 * 64 / 2) + (data->ray[i].case_x - data->map.player_coo->x)
...
int	handle_ray_x_left(t_data *data, int i)
{
	...
	{
		data->ray[i].final_x = 0;
		data->ray[i].final_y = data->ray[i].rx * data->ray[i].delta_y;
		data->ray[i].final_y = round(data->ray[i].coo_y * 64) / 64.0;
		// for (int i = -2; i < 2; i++)
		// {
		// 	for (int j = -2; j < 2; j++)
		// 	{
